window.I18N = (function () {
    let dict = {};
    let lang = localStorage.getItem('lang') || ((navigator.language || '').startsWith('zh') ? 'zh-CN' : 'en-US');
    ...
    async function load(l) {
        try {
            dict = await fetchDict(`/resources/i18n/${l}.json`);
        } catch (e1) {
            try {
                dict = await fetchDict(`/resources/i18n/${l}.json`);
            } catch (e2) {
                console.error('i18n 加载失败', e1, e2);
                dict = {};
            }
        }
        lang = l;
    ...
    function t(key, params) {
        ...
        return typeof val === 'string' ? val.replace(/\{(\w+)\}/g, (_, k) => params?.[k] ?? '') : val;
    }
    ...
    function apply(root = document) {
        ...
        root.querySelectorAll('[placeholder-i18n]').forEach(el => el.setAttribute('placeholder', t(el.getAttribute('placeholder-i18n'))));
        root.querySelectorAll('[title-i18n]').forEach(el => el.setAttribute('title', t(el.getAttribute('title-i18n'))));
    }

    document.addEventListener('DOMContentLoaded', () => load(lang));
    return {
        load, t, apply, get lang() {
            return lang;
        }
    };
})();
